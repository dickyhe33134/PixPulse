...
from routers.auth import ALGORITHM, SECRET_KEY, get_password_hash, oauth2_scheme
...
def get_friends(userid: int = Body(..., embed=True)) -> list[SafeUser]:
    ...
    result = session.exec(
        select(User).where(
            col(User.userid).in_(
                select(HasFriend.friend_id).where(
                    or_(HasFriend.friend_id == userid, HasFriend.userid == userid)
                )
            )
        )
    )
...
def get_close_friends(token: str = Depends(oauth2_scheme)) -> list[SafeUser]:
...
def add_friend(
    ...
    if user is None or friend is None:
        session.close()
        return HTTPException(status_code=400, detail="User or friend does not exist")
...
def send_friend_request(
    sender: int = Body(..., embed=True), receiver: int = Body(..., embed=True)
):
    session = create_session()
    ...
    if sender_user is None or receiver_user is None:
        ...
        return HTTPException(
            status_code=400, detail="Sender or receiver does not exist"
        )
    # Check if the sender and receiver are not the same user
    if sender == receiver:
        session.close()
        return HTTPException(
            status_code=400, detail="Sender and receiver cannot be the same user"
        )
    ...
    if len(result) > 0:
        return HTTPException(status_code=400, detail="Already friends")
    ...
    if len(result) > 0:
        return HTTPException(status_code=400, detail="Friend request already exists")
...
def accept_friend_request(
    ...
    if friend_request is None:
        return HTTPException(status_code=400, detail="Friend request does not exist")
